...
if __name__ == '__main__':
    ...
    # set up file paths
    preds_dir = Path('forecasts_by_horizon') / args.model_config
    preds_dir.mkdir(mode=0o775, parents=True, exist_ok=True)
    preds_file_path = preds_dir / f'{args.forecast_date}-{args.model_config}-{args.horizon}.csv'
    
    params_dir = Path('estimates_by_horizon') / args.model_config
    params_dir.mkdir(mode=0o775, parents=True, exist_ok=True)
    params_file_path = params_dir / f'{args.forecast_date}-{args.model_config}-{args.horizon}.npz'
    
    # exit early if predictions already exist
    if preds_file_path.exists():
        print(f'Skipping estimation for {args.model_config}, {args.forecast_date} horizon {args.horizon}')
        sys.exit(0)
    
    # load data
    data = util.load_data(as_of = args.forecast_date, end_date = args.forecast_date)
    
    # determine effective horizon relative to last observed data date
    last_obs_date = data.date.max()
    horizon_adjust = (pd.Timestamp(args.forecast_date) - last_obs_date).days
    ...
    model_settings = config['model_configs'][args.model_config]
    model_args = model_settings['model_args']
    model_args['h'] = args.horizon + horizon_adjust
    ...
    prev_fit_files = [f for f in params_dir.glob(f'*-{args.model_config}-{args.horizon}.npz')]
    ...
    prev_fit_files = [prev_fit_files[i] \
        for i, fit_date in enumerate(prev_fit_dates) \
            if fit_date < args.forecast_date]
    ...
    model.fit(data,
              tau=tf.constant(np.array([0.025, 0.10, 0.25, 0.50, 0.75, 0.90, 0.975])),
